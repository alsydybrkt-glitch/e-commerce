...
function Product({ item }: { item: ProductType }) {
  ...
  return (
    <Interactive 
      variant="scale"
      className="product-card group relative flex h-full flex-col rounded-3xl border border-border-light bg-surface-primary p-4 transition-all duration-500 hover:border-brand-500/20 hover:shadow-2xl hover:shadow-brand-500/10 dark:bg-slate-900/40 dark:border-slate-800/50 dark:hover:border-brand-400/30 dark:hover:bg-slate-900/80 hover:-translate-y-1.5"
    >
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex gap-1.5 opacity-100 lg:opacity-0 transition-all duration-300 lg:group-hover:opacity-100 lg:group-hover:translate-x-0 translate-x-1 lg:translate-x-4">
          ...
          >
            {favoriteVisible ? <FaHeart className="text-rose-500" /> : <IoHeartOutline className="text-lg" />}
          </button>
          <button
            type="button"
            onClick={handleShare}
