...
export function ProductReviews({ productId }:  { productId?: string | number }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setReviews([
      {
        id: Date.now(),
        author: t("product.reviews.guestUser"),
        rating: newRating,
      ...
      <form onSubmit={handleSubmit} className="mt-6 rounded-2xl bg-white p-4 shadow-sm sm:mt-8 sm:p-6 dark:bg-slate-800/40 dark:ring-1 dark:ring-slate-700/50">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-4 sm:gap-6">
          <div className="flex flex-col gap-1">
            ...
            <select 
              aria-label={t("product.reviews.selectRating")}
              value={newRating} 
            ...
            >
              {[5, 4, 3, 2, 1].map((r) => (
                <option key={r} value={r} className="text-black">{r} {t("product.reviews.stars")}</option>
              ))}
            </select>
          </div>
        </div>
